...
- unfiltered `choose_edf` shift を `generated_schedule_prefix` の one-step unfolding に
  接続する
...
- EDF は同一 deadline の tie-break を candidate list order で解くため、
  ...
  generated schedule prefix induction へ接続するには、service equality と boundary completion
  invariant を induction で維持する必要がある。
