...
この方針だと、今の `GenericDispatchSpec` にそのまま載りますし、後の **Round Robin の土台**にもなります。
...
* `GenericDispatchSpec` は「候補集合から 1 つ選ぶ」形なので、**候補列の順序**を活かすのが自然
...
* `FIFO.v` を追加し、`fifo_generic_spec : GenericDispatchSpec` を作る
...
今の `GenericDispatchSpec` は「選ばれた job は ready」という形なので、まずは **dispatch rule としての FIFO** を作るのがよいです。
...
`GenericDispatchSpec` を満たすには、まず次の 4 本が必要です。
...
この 4 本が揃えば、`mkGenericDispatchSpec` で包めます。
...
`GenericDispatchSpec` だけでも十分ですが、FIFO らしさを明確にするために、
...
* `fifo_generic_spec : GenericDispatchSpec`

を定義します。

ここまで来れば、`Partitioned.v` など **GenericDispatchSpec に依存する層はそのまま FIFO を使える**はずです。
...
FIFO は `GenericDispatchSpec` に乗れば、ここは基本触らなくてよいはずです。
