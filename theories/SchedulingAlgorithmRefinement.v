...
(* ===== 1. Scheduling Algorithm Refinement ===== *)

(* A concrete scheduling algorithm (GenericSchedulingAlgorithm) refines an abstract policy when
   every output of the scheduling algorithm is permitted by the policy, for all inputs.
   This connects the "executable" world (choose functions) to the
...
Definition algorithm_refines_spec
  ...
  forall jobs m sched t candidates,
    policy jobs m sched t candidates
      (spec.(choose) jobs m sched t candidates).

(* ===== 2. Scheduling Algorithm-generated schedules respect the policy ===== *)

(* If a concrete scheduling algorithm refines a policy, then any schedule produced by
...
(* A scheduling algorithm-based schedule also satisfies single_cpu_algorithm_spec_schedule. *)
