...
(* A concrete dispatcher (GenericDispatchSpec) refines an abstract policy when
   ...
   "specification" world (DispatchPolicy predicates). *)
...
(* If a concrete dispatcher refines a policy, then any schedule produced by
   single_cpu_dispatch_schedule respects the policy at every time step. *)
...
Proof.
  ...
  unfold respects_policy_before.
  intros t _.
  exact (single_cpu_dispatch_schedule_respects_policy_at_with
           spec policy candidates_of jobs sched t Href Hrel).
Qed.

(* ===== 3. Scheduler relation inclusion ===== *)

(* A dispatcher-based schedule also satisfies single_cpu_policy_schedule. *)
...
Proof.
  ...
  unfold single_cpu_policy_schedule.
  split.
  - exact (proj1 Hrel).
  - intros t.
    exact (single_cpu_dispatch_schedule_respects_policy_at_with
