...
(* This finite jobset inherits the classic periodic-analysis interpretation:
   jobs are generated by release/cost/deadline structure only. If a later
   development needs blocking-aware jobs, that belongs in an adapter-local or
   workload-specific model rather than this periodic finite-horizon interface. *)

Definition periodic_jobset_nonblocking
    (T : TaskId -> Prop)
    (tasks : TaskId -> Task)
    (offset : TaskId -> Time)
    (jobs : JobId -> Job)
