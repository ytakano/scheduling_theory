...
(* Canonical k-th job generated by periodic task τ. *)
Definition generated_periodic_job
    (tasks : TaskId -> Task)
    (offset : TaskId -> Time)
    (τ : TaskId)
    (k : nat) : Job :=
...
(* A job j is generated by the periodic task model if:
   ...
   This is the classic schedulability-theory job model. It specializes the
   common blocking-aware semantics to non-blocking generated jobs: release,
   cost, and deadline are modeled here, while OS-level wait/block behavior is
   intentionally out of scope. Adapter-local workload models such as Awkernel
   may still model concrete blocking elsewhere. *)
...
Lemma generated_periodic_job_task :
  forall tasks offset τ k,
    job_task (generated_periodic_job tasks offset τ k) = τ.
Proof.
  reflexivity.
Qed.

Lemma generated_periodic_job_index :
  forall tasks offset τ k,
    job_index (generated_periodic_job tasks offset τ k) = k.
Proof.
  reflexivity.
Qed.

Lemma generated_periodic_job_generated :
  forall tasks offset τ k,
    generated_by_periodic_task
      tasks offset
      (fun j => if Nat.eqb j 0 then generated_periodic_job tasks offset τ k
                else generated_periodic_job tasks offset τ k)
      0.
Proof.
  intros tasks offset τ k.
  unfold generated_by_periodic_task.
  cbn [job_task job_index].
  unfold expected_release, expected_abs_deadline.
  rewrite !Nat.eqb_refl.
  simpl.
  split; [reflexivity | split; [reflexivity | lia]].
Qed.

Lemma generated_periodic_job_is_generated :
  forall tasks offset τ k,
    generated_by_periodic_task
      tasks offset
      (fun _ => generated_periodic_job tasks offset τ k)
      0.
Proof.
  intros tasks offset τ k.
  unfold generated_by_periodic_task, generated_periodic_job.
  simpl.
  split; [reflexivity | split; [reflexivity | lia]].
Qed.
